...
def start(jeu,regle):
    if valPileAllumettes(jeu,regle) == 0:
...
def valPileAllumettes(n,regle):
    ...
    for j in range(1,n+1):
        ...
        for coup in regle:
            if coup<=j:
                suiv.append(valeurs[j-coup])
        valeurs.append(mex(suiv))     
    return valeurs[n]
...
def choixOrdinateur(regle,jeu):
            for j in range(len(regle)):
                if jeu >= regle[j]:
                    jeu -= regle[j]
                    if  valPileAllumettes(jeu,regle) == 0:
                        jeu += regle[j]
